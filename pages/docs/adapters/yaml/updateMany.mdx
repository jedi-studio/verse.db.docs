...
# Using the `updateMany` Method with the `json` Adapter
...
### Calling the `updateMany` Method

adding an object to a data file

```js copy showLineNumbers {7,14}
// For example this is our data
const data = [
  { _id: "1234", name: "John" },
  { _id: "5678", name: "Jane" },
];
// this is the filter we are using for this example
const filter = { name: ["John", "Jane"] };
// this is our update query
const updateQuery = { name: "Mike" };
// and this is the dataname (dataname.verse)
...
```json showLineNumbers {4-13}
{
  "acknowledged": true,
  "message": "2 document(s) updated successfully.",
  "results": [
    {
      "_id": "1234",
      "name": "Mike"
